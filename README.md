...
| `scripts/self_check.py` | Самопроверка развёрнутого навыка: конфиг, предметы, выжимки, викторина. |
...
- **«спроси по географии»** / **«проверь по географии»** — берётся запись этого предмета из кеша,
  ...
  То же работает для любого предмета из `subjects.json` (сейчас — география и биология).
...
`subjects.json` и папка `digests/` (выжимки по параграфам) создаются при подготовке предметов викторины (офлайн-пайплайн GeoRag) и **не** попадают в git (см. `.gitignore`); при обновлении их переносят на сервер отдельно и проверяют через `scripts/self_check.py`.
...
git pull                                  # код навыка
source .venv/bin/activate
pip install -r requirements.txt           # обновить зависимости (quiz-library и др.)
```

Обновление данных предметов (выжимки + `subjects.json`) — файлы **приватные**, их не ставить из git, а переносить со своей машины (например через `scp`):

```bash
# на своей машине (Windows), где лежат свежие digests/ и subjects.json:
scp -r digests user@server:~/alice-homework/
scp subjects.json user@server:~/alice-homework/
```

Независимо от того, был ли `git pull`, **надо перезапустить процесс**, чтобы конфиг и данные перечитались:
...
Проверка обновления — самопроверка навыка:
...
Скрипт печатает результат каждой проверки (`[OK ]` / `[WARN]` / `[FAIL]`) и возвращает
код `0`, если всё в порядке, `1` — если есть проблема. Проверки:

- `config.json` есть и читается, поля СГО и `skill_id` заполнены;
- викторина включена (`llm.api_key`) или честно выключена (предупреждение, не ошибка);
- каждый предмет из `subjects.json`: выжимка существует, читается как JSON, в ней корректные параграфы;
- тестовая запись «пересказать параграф 6» распознаётся именно как 6 (по `paragraph_patterns` предмета);
- `quiz-library` установлена и импортируется.

После `git pull` + обновления данных + `self_check.py` (все `[OK ]`) навык обновлён.
...
Полный набор (**138 тестов**) покрывает:
...
- `tests/test_self_check.py` — все проверки `scripts/self_check.py`.
...
├── tests/                 # 12 файлов, 138 тестов
├── scripts/self_check.py  # самопроверка навыка перед/после обновления
